...
def load_video(file, fps=1., time_range=None, num_threads=0):
    ...
    if time_range is None:
        ...
        idx_s = round(time_range[0] * vr.get_avg_fps())
        idx_e = min(round(time_range[1] * vr.get_avg_fps()), len(vr)-1)
        num_steps = (time_range[1] - time_range[0]) * fps
        frame_idx = np.linspace(idx_s, idx_e, round(num_steps), dtype=int)
...
def get_media_length(file):
    ...
    duration = run(cmd, capture_output=True, check=True).stdout
