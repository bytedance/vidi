...
def success_overlap(results):
    ...
    for i in range(n_query):
        try:
            iou[i] = overlap_ratio(np.array(results[i]['answer']), results[i]['gt'])
        except:
            pdb.set_trace()
...
def print_result(model_name, nums, precision, recall, auc):
    ...
    print(f"Precision: {precision*100:.2f}%, Recall: {recall*100:.2f}%, IoU: {auc*100:.2f}%")
    print("-----------------------------------------------------")
...
def print_attribute_result(precision_scores, recall_scores, iou_scores, attributes):
    ...
    df_formatted = df.applymap(lambda x: f"{x*100:.2f}%")
    print(df_formatted)
    df_formatted.to_csv("results/results_table.csv")

# evaluate results
def evaluate_results(output_dir, res_path, gt_path):
    all_results = {}
    attributes = np.array([
        "ultra-short", "short", "medium", "long", "ultra-long", "keyword", "phrase", "sentence", "vision", "audio", "vision+audio", "overall"
    ])
    root = "" # path of results of compared methods
    compared_methods = [res_path] + ['results_Gemini-3-Pro-Prev.json', 'results_GPT-5.json']
    compared_paths = [osp.join(root, res_name) for res_name in compared_methods]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred_path", default='results_Vidi2.json', type=str, help="The path to file containing prediction.")
    parser.add_argument("--gt_path", default='VUE-TRv2_ground_truth.json', type=str, help="The path to file containing ground-truth.")
