...
from model.img_utils import get_anyres_image_grid_shape
from model.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX
from model.mm_vision import (
    CLIPVisionTower, SiglipVisionTower, LearnablePosEmbd, Conv2DPool
)
from model.mm_audio import WhisperAudioTower
from model.mm_layer import MLP, RMSNorm, rms_norm
    ...
    def __init__(self, config):
        ...
        if config.mm_input_type == "video":
            ...
            self.mm_rand_aud_pool = nn.Conv1d(
                self.mm_aud.hidden_size, self.mm_aud.hidden_size, bias=False,
                kernel_size=config.mm_audio_pool_size, stride=config.mm_audio_pool_size
            )
            self.mm_rand_aud_projector = MLP(
                config.mm_projector_type, self.mm_aud.hidden_size, config.hidden_size
            )
...
class DattnMMMixin(MMMixin):
    def encode_video_images(self, images):
        ...
        with torch.set_grad_enabled(self.config.train_vis and self.training):
    ...
    def encode_video_audios(self, audios, audio_sizes):
        ...
        with torch.set_grad_enabled(self.config.train_aud and self.training):
            audio_features = splitted_call(
                self.get_model().mm_aud, concat_audios, self.config.mm_splits
            )
        ...
        audio_features = splitted_call(
            self.get_model().mm_rand_aud_pool, audio_features, self.config.mm_splits
        )
        audio_features = audio_features.permute(0, 2, 1)
